...
// ─── Tree rendering ────────────────────────────────────────────────────────────
function colorizeNodeType(nodeType, rows, analyzed) {
...
export function renderTree(node, indent = 0, isLast = true) {
    const lines = [];
    const prefix = indent === 0 ? "" : "  ".repeat(indent - 1) + (isLast ? "└─ " : "├─ ");
    const analyzed = node["Actual Total Time"] !== undefined;
    const nodeType = colorizeNodeType(node["Node Type"], node["Actual Rows"] ?? node["Plan Rows"], analyzed);
    ...
    let extra = "";
    if (node["Index Name"])
        extra += chalk.dim(` idx=${node["Index Name"]}`);
    if (node["Filter"])
        extra += chalk.dim(` filter=${node["Filter"].slice(0, 40)}`);
    if (node["Sort Key"])
        extra += chalk.dim(` sort=[${node["Sort Key"].join(", ")}]`);
    if (node["Hash Batches"] && node["Hash Batches"] > 1)
        extra += chalk.yellow(` batches=${node["Hash Batches"]}`);
...
// ─── Top-level analyze ─────────────────────────────────────────────────────────
export function analyze(rawJson, query) {
...
export function formatSummary(analysis) {
    ...
    if (analysis.summary.bufferHits > 0 || analysis.summary.bufferMisses > 0) {
        const hitRate = analysis.summary.bufferHits + analysis.summary.bufferMisses > 0
