...
import type { PlanNode, ExplainResult } from "../types.js";

// ─── Mock data ─────────────────────────────────────────────────────────────────

const seqScanNode: PlanNode = {
  "Node Type": "Seq Scan",
  "Relation Name": "users",
  "Alias": "users",
  "Startup Cost": 0,
...
const simpleResult: ExplainResult = {
  Plan: seqScanNode,
  "Planning Time": 0.5,
  "Execution Time": 12.5,
};

const nestedResult: ExplainResult = {
  Plan: {
...
describe("buildSummary", () => {
  ...
  test("no seq scans for index scan only", () => {
    const result: ExplainResult = { Plan: indexScanNode, "Planning Time": 0.1, "Execution Time": 0.07 };
...
describe("buildRecommendations", () => {
  ...
  test("no warning for seq scan on small table", () => {
    const smallSeq: PlanNode = {
      ...seqScanNode,
      "Plan Rows": 50,
      "Actual Rows": 50,
    };
    const result: ExplainResult = { Plan: smallSeq };
  ...
  test("flags hash batches > 1", () => {
    const result: ExplainResult = { Plan: hashJoinNode };
    const recs = buildRecommendations(result);
  ...
  test("suggests index for sort node", () => {
    const result: ExplainResult = { Plan: sortNode };
    const recs = buildRecommendations(result);
  ...
  test("warns about bad row estimates", () => {
    const badEstimate: PlanNode = {
      ...seqScanNode,
      "Plan Rows": 1,
      "Actual Rows": 50000,
    };
    const result: ExplainResult = { Plan: badEstimate };
    const recs = buildRecommendations(result);
  ...
  test("no recommendations for fast index scan", () => {
    const result: ExplainResult = { Plan: indexScanNode };
    const recs = buildRecommendations(result);
